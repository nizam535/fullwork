...
export function TransportForm({ onSuccess }: { onSuccess?: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert('Error submitting transport record: ' + error.message);
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div>
          ...
          <select
            ...
            className={`w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent ${
              formData.material_transported === "Aggregate's Rehandling" ? 'bg-gray-100' : ''
            }`}
          >
        ...
        <div>
          ...
          <select
            ...
            className={`w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent ${
              !formData.from_location || formData.material_transported === "Aggregate's Rehandling" ? 'opacity-50 cursor-not-allowed' : ''
            }`}
          >
            <option value="">
